...
**Content rollout is complete — every route and every shared component is migrated.** A page counts as migrated when its `export const metadata` is an async `generateMetadata({params})` using `getTranslations`, its component is `async function Page({params})` using `getTranslations` (or `useTranslations` if it's a client component), and every user-facing string is a key in `src/messages/{pt-BR,en-US,es}.json` instead of hardcoded JSX text. All routes follow this pattern now, plus the shared `Menu`/`Footer`/`Committee`/`Form`/`Pagination`/`ContactBanner`/`MediaCard` components. When adding a *new* page or component, follow the pattern in `src/app/[locale]/contato/page.tsx` (richer example — form + subject options where the emailed `value` stays pt-BR but the visible `label` translates) or `src/app/[locale]/parceiros/page.tsx` (simpler, no form). The one deliberate exception: `/midia` blog post *content* (title/excerpt/body from `db/temp.json`) has no locale dimension yet and still renders pt-BR regardless of locale — see "Internationalization (i18n branch, content rollout complete)" in `.claude/NOTES.md`.
...
- **`next-sitemap` emits all three locales (fixed).** `next-sitemap`'s route discovery can't resolve the `[locale]` dynamic segment, so relying on it alone produced a pt-BR-only sitemap. `next-sitemap.config.js` now builds the full URL list itself via `additionalPaths`: a static route list × `LOCALE_PREFIXES` (duplicated from `routing.ts` — keep both in sync) plus the blog slugs read directly from `temp.json`. `exclude: ['/*']` suppresses the (contributing-nothing) auto-discovery so `additionalPaths` is the only source. The committed `public/sitemap*.xml` now carries all pt-BR/en-US/es URLs; only `lastmod` timestamps change between builds — revert those before committing, same as before.
