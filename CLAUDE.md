...
**Color tokens.** The live, canonical surface is the olive greens (`green-dark`/`green-pale`/`green-light`). The client-confirmed **full brand palette** is also tokenized (the "Brand palette" block in `theme.pcss`): `creme`/`citron`/`olive` (col 1), `beige`/`bordeaux`/`brule` (col 2), `ciano`/`sereno`/`safira` (col 3) → utilities like `bg-bordeaux`, `from-safira to-sereno`. The greens map onto it (`green-dark` = Olive, `green-light` = Citron). **`safira #01083A` / `sereno #323F70` are exact (the Bio+/ICT tones); `creme`/`beige`/`bordeaux`/`brule`/`ciano` are ESTIMATED from the swatch image and flagged `/* confirm */`** — get exact hexes from the client before relying on them. The Bio+/ICT entity uses the blues (Safira/Sereno + the `blue` Button variant + the navy gradient on the `/sobre` ICT card); its identity is a multi-colour *degradê* (a `Grainient` with the Bio+ palette is the natural background for `/sobre/ict-aether-bio`).
...
**Navigation links live in `src/utils/routes.js`, but the three surfaces are now decoupled** (they used to share one `navLinks`): **`navLinks`** feeds the **mobile (fullscreen) menu** *and* the **`Footer`** (the full sitemap); **`headerLinks`** is a deliberately trimmed subset for the **desktop header only** (currently Sobre / Mídia / Inscreva seu Projeto / Contato — the header isn't the whole sitemap). Both are in `Menu` (`headerLinks` → the `max-lg:hidden` `<ul>`, `navLinks` → the fs `<aside>`). The header no longer filters `home` — just don't put a `home` entry in `headerLinks`. Edit each list independently; a destination shared between them must be changed in both. Anchors (`#…`) scroll the home via `useAnchorScroll`; routes (`/…`) navigate; the `home` entry (`/`, in `navLinks` only) navigates home from other pages but scrolls to top when you're already on home. When a section becomes a real page (e.g. `/sobre`), flip its entry from `#…` to `/…`. The **Footer is slated to move to its own grouped array** (the client wants columns; not built yet — it still maps `navLinks`). In-page contact CTAs (Banner/About "Entre em contato") point at `/contato`.
...
- **`FileUpload`** now exists (built for `/inscreva-seu-projeto`). PDF-only ≤15MB by default (`accept`, `maxSizeMB` props); RHF `validate` enforces type + size client-side with the spec's pt-BR messages; shows a dashed dropzone → selected filename + size + remove button; `microcopy` prop for the helper line. The field registers a real `<input type="file">` so `data[name]` is a `FileList` — the `Form` `isFormData` loop unwraps `FileList → file` and skips empty values, so it must be submitted with **`isFormData`** on the `Form`.
