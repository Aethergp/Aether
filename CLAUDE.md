...
- **Internal `Link`/`Button` navigation carries the locale prefix (fixed).** Every internal `href` on the site (`Menu`, `Footer`, `Button`, `ContactBanner`, `MediaCard`, `ProjectForm`, the `/sobre/ict-aether-bio` "Saiba mais" link) is resolved through `getPathname({ href, locale })` from `@/i18n/navigation` before being handed to `next-transition-router`'s `Link` — `useLocale()` supplies the locale in Client Components, and the already-in-scope `locale` (from `params`) is used directly in async Server Components. `Button` itself has no `'use client'` pragma and stays a plain shared component (next-intl's `useLocale()`/`useTranslations()` work in both Server and Client Component contexts, so it renders correctly whichever side imports it). Clicking any nav item or CTA while browsing `/en-us/...` or `/es/...` now stays on that locale instead of dropping back to the pt-BR URL.
- **`next-sitemap` emits all three locales (fixed).** `next-sitemap`'s route discovery can't resolve the `[locale]` dynamic segment, so relying on it alone produced a pt-BR-only sitemap. `next-sitemap.config.js` now builds the full URL list itself via `additionalPaths`: a static route list × `LOCALE_PREFIXES` (duplicated from `routing.ts` — keep both in sync) plus the blog slugs read directly from `posts.json`. `exclude: ['/*']` suppresses the (contributing-nothing) auto-discovery so `additionalPaths` is the only source. The committed `public/sitemap*.xml` now carries all pt-BR/en-US/es URLs; only `lastmod` timestamps change between builds — revert those before committing, same as before.
