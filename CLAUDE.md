...
**Content rollout is per page, not all-at-once.** A page has been "migrated" when its `export const metadata` becomes an async `generateMetadata({params})` using `getTranslations`, its component becomes `async function Page({params})` using `getTranslations` (or `useTranslations` if it's a client component), and every user-facing string is a key in `src/messages/{pt-BR,en-US,es}.json` instead of hardcoded JSX text. **Migrated so far: `404`, `/contato`, `/parceiros`.** Every other route still has hardcoded pt-BR copy in its JSX — visiting `/en-us/sobre` today still renders Portuguese body copy (only the global chrome/shell is locale-aware). This is intentional incremental rollout, not a bug: un-migrated pages simply show pt-BR regardless of locale until their turn comes. When migrating a page, follow the pattern in `src/app/[locale]/contato/page.tsx` and `src/app/[locale]/parceiros/page.tsx`.
...
- **Known gap: internal `Link`/`Button` navigation does not carry the locale prefix.** Every internal link on the site (`Menu`, `Footer`, `Button`) goes through `next-transition-router`'s `Link` (for the page-transition curtain animation), fed directly with the unprefixed `pages.x` constants from `routes.js` — not next-intl's locale-aware `Link`. So clicking any nav item or CTA while browsing `/en-us/...` or `/es/...` currently drops back to the pt-BR URL. Fixing this properly means auditing every `href={pages.x}` usage and either swapping the Link source or computing the href through next-intl's `getPathname`, which hasn't been done yet — treat it as a known follow-up, not a regression to silently patch.
- **Known gap: `next-sitemap` doesn't understand the `[locale]` segment and currently emits an empty sitemap.** `next-sitemap`'s route discovery doesn't resolve the dynamic-segment `generateStaticParams` output, so `npm run build`'s postbuild step produces a `sitemap-0.xml`/`sitemap.xml` with zero URLs instead of the real page list. This needs a custom `next-sitemap.config.js` (`additionalPaths`, likely built from `routing.locales` × the route list) or a switch to Next's native `sitemap.ts` route convention — not done yet. Don't commit a build's regenerated `public/sitemap*.xml` until this is fixed; the committed one is normal (pt-BR-only) copy.
