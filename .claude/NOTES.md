...
- **Palette / brand manual.** The implemented, live theme is olive-green (`green-dark`/`green-pale`/
  ...
  `/pd`) call for the Safira/Azul Sereno complementary palette. **The Bio+ tokens now exist**:
  `--color-ict-safira #01083A` / `--color-ict-azul #323F70` in `theme.pcss` (first used on the `/sobre`
  ICT pillar card). Bordeaux/Verde Citron/Crème (main brand) are still NOT in the theme - confirm against
  `_docs/original/` whether they should be, and decide how far the Bio+ palette extends (whole
  `/sobre/ict-aether-bio` page vs. accents) when building that page.
