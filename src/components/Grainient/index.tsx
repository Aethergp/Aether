...
// vendored from React Bits (https://reactbits.dev) - animated WebGL grainient
// background. ported to TS; container styled with Tailwind instead of the
// shipped css. green-toned palette set by the caller via the color props.
...
// keep renderer/program alive across re-renders so the prop->uniform effect can
...
export default function Grainient({
	...
	// sync props to uniforms - zero GPU cost, no teardown
	useEffect(() => {
		const container = containerRef.current
		if (!container) return
	...
	return <div ref={containerRef} className={clsx('relative w-full h-full overflow-hidden', className)} />
}
